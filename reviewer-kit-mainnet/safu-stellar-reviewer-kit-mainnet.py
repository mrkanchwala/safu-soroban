#!/usr/bin/env python3
"""
SAFU Stellar Tranche #3 — MAINNET reviewer self-test kit.

Lets an SCF reviewer independently verify and then TRIGGER a real payout
from SAFU's deployed Soroban ProtectionPool on Stellar MAINNET, on their own
schedule, without depending on a pre-recorded video capture.

This is the same kit design used at Tranche #2 (testnet), pointed at the live
mainnet contract. Two commands:
...
MAINNET, REAL CAPITAL. The staker keys in the accompanying config control
real, founder-funded Stellar accounts operating inside SAFU's own capped test
pool (pool_cap = 40,000 XLM; max single stake 500 XLM at that cap). A staker
key can trigger a payout, but the payout can only ever land at the
beneficiary address committed on-chain at stake time — it cannot be
redirected. That is why publishing the staker key is safe; the matching
beneficiary secret key is deliberately NOT included in this kit.

The contract itself is pre-audit — SAFU applied to the SCF Audit Bank
immediately after Tranche 2 was approved and is awaiting a matched auditor.
See this tranche's submission notes for the current status.

Usage:
  python3 safu-stellar-reviewer-kit-mainnet.py check
  python3 safu-stellar-reviewer-kit-mainnet.py check --claim claim1
...
def load_config(path):
    if not os.path.exists(path):
        sys.exit(
            ...
            "Expected `mainnet-reviewer-wallets.json` next to this script, or pass --config."
        )
...
def read_contract(server, contract_id, source_pub, fn, params, passphrase):
    """Simulate a call purely to read its return value. Nothing is signed or sent."""
    src = server.load_account(source_pub)
    tx = (
        TransactionBuilder(src, passphrase, base_fee=1000)
...
def vesting_snapshot(claim, now_ledger):
    """Reproduce claim.rs's vesting math exactly, so the numbers shown match
    what the contract will actually pay."""
...
def outflow_cap(server, contract_id, source_pub, claim, passphrase):
    """Pool-wide daily payout ceiling — claim.rs's dynamic outflow logic.
    ...
    """
    total_staked = read_contract(server, contract_id, source_pub, "get_total_staked", [], passphrase)
    total_allocated = read_contract(server, contract_id, source_pub, "get_total_allocated", [], passphrase)
...
def expert_tx(cfg, txid):
    return f"https://stellar.expert/explorer/{cfg['network']}/tx/{txid}"


def expert_contract(cfg, cid):
    return f"https://stellar.expert/explorer/{cfg['network']}/contract/{cid}"
...
def cmd_check(cfg, args):
    ...
    pool_staked = read_contract(server, contract_id, any_pub, "get_total_staked", [], passphrase)
    pool_allocated = read_contract(server, contract_id, any_pub, "get_total_allocated", [], passphrase)

    log("=" * 72)
    log("SAFU Stellar ProtectionPool - claim status (read-only) - MAINNET")
    log("=" * 72)
    log(f"contract      : {contract_id}")
    log(f"               {expert_contract(cfg, contract_id)}")
    log(f"network       : {cfg['network']} (real capital)")
    ...
    for c in selected:
        ...
        cap = outflow_cap(server, contract_id, source_pub, claim, passphrase)
    ...
    log("")
    log("NOTE: mainnet transactions are permanent, not reset like testnet evidence.")
    log("NOTE: the contract is pre-audit -- SAFU applied to the SCF Audit Bank 2026-09-02")
    log("      and is awaiting a matched auditor. See this tranche's submission notes.")
...
def cmd_stream(cfg, args):
    ...
    v = vesting_snapshot(claim, now_ledger)
    log(f"claim [{c['label']}] {c['claim_id'][:16]}...  (MAINNET, real capital)")
    ...
    # `claim.wallet.require_auth()` -- the STAKER authorizes, not the
    # beneficiary. The beneficiary is checked separately against the hash
    # committed at stake time.
    source = server.load_account(staker_kp.public_key)
    tx = (
        TransactionBuilder(source, passphrase, base_fee=1000)
        .append_invoke_contract_function_op(
    ...
    if op_auth:
        tx.transaction.operations[0].auth = [
            authorize_entry(e, staker_kp, valid_until, passphrase)
            for e in op_auth
        ]
    ...
    log("Submitting a REAL mainnet transaction...")
...
def main():
    default_cfg = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "mainnet-reviewer-wallets.json"
    )

    p = argparse.ArgumentParser(
        description="SAFU Stellar T3 reviewer self-test kit (MAINNET, real capital).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p.add_argument("--config", default=default_cfg, help="path to mainnet-reviewer-wallets.json")
